...
IMPORT_COLUMNS = [
    ...
    "profit_amount", "profit_rate", "holding_share", "latest_price", "target_min_ratio",
    "target_max_ratio", "risk_level", "note", "duplicate_action",
]
HOLDING_FIELDS = set(IMPORT_COLUMNS) - {"duplicate_action"}
...
def parsed_to_drafts(parsed: Iterable[Mapping]) -> pd.DataFrame:
    ...
    for item in parsed:
        ...
        rows.append({
            ...
            "profit_rate": safe_float(item.get("profit_rate"), rate), "holding_share": item.get("holding_share"),
